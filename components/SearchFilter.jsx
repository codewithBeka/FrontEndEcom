...
const filters = [
  ...
  },
  {
    ...
    options: [
      ...
      { value: '501-1000', label: '$501 - $1000', checked: false },
      { value: '1001-1500', label: '$1001 - $1500', checked: false },
      { value: '1501+', label: '$1501+', checked: false },
    ],
  },
];
...
export default function SearchFilter() {
  ...
  const [selectedFilters, setSelectedFilters] = useState({ brand: [], price: [] });
  const history = useHistory();

  /**
   * Updates the URL with the current selected filters
   * @private
   */
  const updateUrl = () => {
    const params = new URLSearchParams();
    if (selectedFilters.brand.length) params.append('brands', selectedFilters.brand.join(','));
    if (selectedFilters.price.length) params.append('price', selectedFilters.price.join(','));
  ...
  useEffect(() => {
    ...
    const price = params.get('price') ? params.get('price').split(',') : [];
    setSelectedFilters({ brand: brands, price });
  }, []);
